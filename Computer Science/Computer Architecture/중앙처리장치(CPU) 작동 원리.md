...
- MAR(Memory Address Register, 메모리 주소 레지스터) : 읽기와 쓰기 연산을 수행할 주기억장치 주소 저장
- PC(Program Counter, 프로그램 카운터) : 다음에 수행할 명령어 주소 저장
- IR(Indtruction Register, 명령어 레지스터) : 현재 실행 중인 명령어 저장
- MBR(Memory Buffer Register, 메모리 버퍼 레지스터) : 주기억장치에서 읽어온 데이터 or 저장할 데이터 임시 저장
- AC(Accumulatior, 누산기) : 연산 결과 임시 저장
